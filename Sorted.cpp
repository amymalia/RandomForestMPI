...
int getNodeDepth (cv::ml::DTrees::Node::Node node, std::vector<cv::ml::DTrees::Node::Node> nodeList) {
	if ((node.right == -1) && (node.left == -1)) 
		return 1;
	return 1 + getNodeDepth(nodeList[node.left], nodeList) + getNodeDepth(nodeList[node.right], nodeList);
}

int getNodeDepth (int index, std::vector<cv::ml::DTrees::Node::Node> nodeList, std::vector<int> treeList) {
	if (index == treeList.size() - 1) {
		return nodeList.size() - treeList[index];
	}
	return treeList[index + 1] - treeList[index];
}
...
void compare(std::vector<int> a, int i, int j, std::vector<cv::ml::DTrees::Node::Node> nodeList) {
    if (getNodeDepth(i, nodeList, a) > (getNodeDepth(j, nodeList, a)))
		printf("Compare %d with %d.\n", getNodeDepth(i, nodeList, a), getNodeDepth(j, nodeList, a));
        exchange(a, i, j);
}
 ...
 */
void oddEvenMerge(std::vector<int> a, int lo, int n, int r, std::vector<cv::ml::DTrees::Node::Node> nodeList) {
    int m = r * 2;
    if (m < n) {
        oddEvenMerge(a, lo, n, m, nodeList); // even subsequence
        oddEvenMerge(a, lo + r, n, m, nodeList); // odd subsequence
        for (int i = lo + r; i + r < lo + n; i += m)
            compare(a, i, i + r, nodeList);
    } else
        compare(a, lo, lo + r, nodeList);
}
 ...
 */
void oddEvenMergeSort(std::vector<int> a, int lo, int n, std::vector<cv::ml::DTrees::Node::Node> nodeList) {
    if (n > 1) {
        int m = n / 2;
        oddEvenMergeSort(a, lo, m, nodeList);
        oddEvenMergeSort(a, lo + m, m, nodeList);
        oddEvenMerge(a, lo, n, 1, nodeList);
    }
}

std::vector<int> mergeSort(std::vector<int> a, std::vector<cv::ml::DTrees::Node::Node> nodeList) {
    int i, n = sizeof(a) / sizeof(a[0]);
    for (i = 0; i < n; i++)
        std::cout << std::setw(3) << a[i];
    std::cout << std::endl;
    oddEvenMergeSort(a, 0, n, nodeList);
    for (i = 0; i < n; i++)
        std::cout << a[i] << "  *   ";
    std::cout << std::endl;
									 ...
									 int num_procs)
{
	...
	if (my_rank == 0) {
		...
		//Sort trees by size
		std::vector<int> treeList = forest->getRoots();
		printf("treeList size %lu \n", treeList.size());
		std::vector<cv::ml::DTrees::Node::Node> nodeList = forest->getNodes();
		...
		for (i = 0; i < treeList.size(); i++){
			printf ("DEPTH: %d \n", getNodeDepth(i, nodeList, treeList));
		}
		
		treeList = mergeSort(treeList, nodeList);
		for (i = 0; i < treeList.size(); i++){
			printf ("DEPTH: %d \n", getNodeDepth(i, nodeList, treeList));
		}
	...
	MPI_Reduce(&train_hr, &train_hr, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&test_hr, &test_hr, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
						...
						int num_procs)
{
    ...
    test_and_save_classifier(model, model, data, responses, ntrain_samples, 0, filename_to_save, my_rank, num_procs);
